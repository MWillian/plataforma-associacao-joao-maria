...
Your data contract is the heart of your application. It lives at [`server/prisma/schema.prisma`](server/prisma/schema.prisma) and describes your models:
...
export default definePrismaConfig({
  orm: ormConfig({
    contract: './server/prisma/schema.prisma',
    db: {
...
| File | Purpose |
|---|---|
| [`server/prisma/schema.prisma`](server/prisma/schema.prisma) | Your data contract — define your models here |
| [`prisma.config.ts`](prisma.config.ts) | CLI configuration |
| [`server/prisma/db.ts`](server/prisma/db.ts) | Database client — `import { db } from './server/prisma/db'` |
| `server/prisma/contract.json` | Compiled contract (generated) |
| `server/prisma/contract.d.ts` | Contract types (generated) |

### Workflow

1. Edit [`server/prisma/schema.prisma`](server/prisma/schema.prisma) to add or change models.
